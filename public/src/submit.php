...
// === ALLOW AJAX & NORMAL POST (but require X-Requested-With for AJAX) ===
...
try {
    ...
    $sea = [
        ...
        'version'        => ($action === 'create') ? '1' : (intval($existing['version'] ?? 0) + 1),
    ...
    if (!empty($_FILES['attachments']['name'][0])) {
        ...
        for ($k = 0; $k < count($_FILES['attachments']['name']); $k++) {
            ...
            $webUrl = $webBase . '/' . rawurlencode($safeName);

            if (file_exists($targetPath) && !in_array($safeName, $overwrite)) {
                $conflicts[] = $safeName;
                continue;
            }
    ...
    // === CONFLICT? RETURN FOR CONFIRMATION ===
    if (!empty($conflicts)) {
        if ($isAjax) {
            ...
            header('Location: ../index.html?msg=conflict');
            exit;
        }
    }

    $sea['attachments'] = array_unique(array_merge($keepAttachments, $newAttachments));
    ...
    $response['message'] = $action === 'create' ? 'SEA created successfully!' : 'SEA updated successfully!';
    $response['sea_id'] = $short_id;
    $response['attachments'] = $sea['attachments'];
    ...
    if ($isAjax) {
        ...
        header('Location: ../index.html?id=' . $short_id . '&msg=' . $msg);
        exit;
    }
    ...
    if ($isAjax) {
        http_response_code(200); // Avoid 500 if possible
        echo json_encode($response);
        exit;
    } else {
